    ...
    for (
        ...
        if distances:
            ...
            unique_distances, counts = np.unique(
                sorted_distances, return_counts=True
            )

            # Populate the shortest_dist and second shortest_dist distances
            if len(unique_distances) > 0:
                first_second_dist_info[label]["shortest_dist"] = (
                    unique_distances[0]
                )
                first_second_dist_info[label]["counts"][
                    unique_distances[0]
                ] = counts[0]
            if len(unique_distances) > 1:
                first_second_dist_info[label]["second_shortest_dist"] = (
                    unique_distances[1]
                )
                first_second_dist_info[label]["counts"][
                    unique_distances[1]
...
def extract_best_labels(data):
    ...
    for label, details in data.items():
        ...
        total_distances[element]["shortest_counts"].append(
            details["counts"][details["shortest_dist"]]
        )
        ...
        if element not in best_labels:
            ...
            if (details["shortest_dist"] < current_best["shortest_dist"]) or (
                ...
                and details["second_shortest_dist"]
                < current_best["second_shortest_dist"]
            ):
                best_labels[element] = (label, details)
    ...
    for element, (label, details) in best_labels.items():
        ...
        total_second_shortest = sum(
            total_distances[element]["second_shortest_counts"]
        )
...
def extract_shortest_dist_with_tol(
    best_label_dist_info, connections, tol=0.05
):
    ...
    for element, info in best_label_dist_info.items():
        ...
        best_site_min_dist = best_label_dist_info[element]["details"][
            "shortest_dist"
        ]
...
def extract_avg_shortest_dist_with_tol(connection_data, tol=0.05):
    ...
    for site_label, connections in connection_data.items():
        ...
        site_result[site_label] = {
            "shortest_dist_count_within_tol": count_within_tol
        }

    # Step 2: Aggregate counts by element and compute averages
    avg_result = defaultdict(
        lambda: {"total_shortest_dist_within_tol_count": 0}
    )
    ...
    for site_label, result in site_result.items():
        ...
        if site_label in element_to_site_labels[element]:
            avg_result[element][
                "total_shortest_dist_within_tol_count"
            ] += result["shortest_dist_count_within_tol"]
...
def get_avg_second_by_first_shortest_dist_ratio(
    site_dist_info, connection_data
):
    """Compute the ratio of the second shortest distance to the shortest
    distance for each site label."""
    ratio_total_result = defaultdict(
        lambda: {"total_second_by_first_shortest_dist": 0.0}
    )
    ...
    for site_label, data in site_dist_info.items():
        ...
        ratio_total_result[element][
            "total_second_by_first_shortest_dist"
        ] += ratio
    ...
    for element, result in ratio_total_result.items():
        ...
        if num_sites > 0:
            avg_ratio = (
                result["total_second_by_first_shortest_dist"] / num_sites
            )
        else:
            avg_ratio = 0.0

        ratio_avg_result[element] = {
            "avg_second_by_first_shortest_dist": avg_ratio
        }
